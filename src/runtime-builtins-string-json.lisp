...
(defun %php-json-parse-value (s pos)
  ...
  (let ((ch (char s pos)))
    (cond
      ...
      ((and (<= (+ pos 4) (length s)) (string= s "null" :start1 pos :end1 (+ pos 4)))
       (values +php-null+ (+ pos 4)))
      ((and (<= (+ pos 4) (length s)) (string= s "true" :start1 pos :end1 (+ pos 4)))
       (values t (+ pos 4)))
      ((and (<= (+ pos 5) (length s)) (string= s "false" :start1 pos :end1 (+ pos 5)))
       (values nil (+ pos 5)))
      ((or (digit-char-p ch) (char= ch #\-))
       (let ((end pos))
         (when (char= ch #\-) (incf end))
         (loop while (and (< end (length s))
                          (or (digit-char-p (char s end))
                              (member (char s end) '(#\. #\e #\E #\+ #\-))))
               do (incf end))
               ...
               (if (char= ch #\\)
                   (let ((esc (char s pos)))
                     ...
                     (case esc
                       ...
                       (#\u (let ((code (parse-integer s :start pos
                                                        :end (min (length s) (+ pos 4)) :radix 16)))
                              (incf pos 4)
