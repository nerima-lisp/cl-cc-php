...
(describe
  ...
  (it-sequential
    "php-e2e-postfix-incdec-on-places"
    (expect
      (%php-run-capture
      ...
      "2")
    (expect
      (%php-run-capture "<?php class C{ public $n=10; } $o=new C(); echo $o->n++;")
      :to-equal
      "10")
    (expect
      (%php-run-capture
        "<?php class C{ public $n=5; } $o=new C(); $o->n--; echo $o->n;")
      :to-equal
      "4")
    (expect
      (%php-run-capture
      ...
      "3")
    (expect
      (%php-run-capture "<?php $a=[0]; $a[0]++; $a[0]++; echo $a[0];")
      :to-equal
      "2")
    (expect
      (%php-run-capture "<?php $a=[5]; $old=$a[0]++; echo $old.'-'.$a[0];")
      :to-equal
      "5-6")
    (expect
      (%php-run-capture "<?php $x='7'; $old=$x++; echo $old.'-'.$x;")
      :to-equal
      "7-8")
    (expect (%php-run-capture "<?php $x=null; ++$x; echo $x;") :to-equal "1")
    (expect (%php-run-capture "<?php $x=true; --$x; echo $x;") :to-equal "0"))
  (it-sequential
    "php-e2e-incdec-undefined-variable"
  ...
  (it-sequential
    ...
    (expect
      ...
      "1hello2"))
  (it-sequential
    "php-e2e-declare-directives"
    (expect
      (%php-run-capture "<?php declare(strict_types=1); echo 7;")
      :to-equal
      "7")
    (expect
      (%php-run-capture "<?php declare(ticks=1) { echo 'A'; echo 'B'; } echo 'C';")
      :to-equal
      "ABC")
    (expect
      (%php-run-capture "<?php declare(ticks=1) echo 'X'; echo 'Y';")
      :to-equal
      "XY")
    (expect
      (%php-run-capture
        "<?php declare(ticks=1): echo 'A'; echo 'B'; enddeclare; echo 'C';")
      :to-equal
      "ABC"))
  (it-sequential
    "php-e2e-output-buffering-builtins"
    (expect
      (%php-run-capture
        "<?php ob_start(); echo 'a'; $s=ob_get_clean(); echo $s; echo 'b';")
      :to-equal
      "ab")
    (expect
      (%php-run-capture
    ...
    (expect
      (%php-run-capture
        "<?php ob_start(); ?>html<?php $s=ob_get_clean(); echo $s.'!';")
      :to-equal
      "html!"))
  (it-sequential
    "php-e2e-ini-get-set-and-error-reporting"
    (expect
      (%php-run-capture "<?php echo ini_get('default_charset');")
      :to-equal
      "UTF-8")
    (expect
      (%php-run-capture
    ...
    (expect
      ...
      "false"))
  (it-sequential
    "php-e2e-error-and-exception-handler-registration"
    (expect
      (%php-run-capture
    ...
    (expect
      ...
      "eh:ok"))
  (it-sequential
    "php-e2e-error-suppression-operator"
    (expect
    ...
    (expect
      ...
      "1:32767"))
  (it-sequential
    "php-e2e-date-default-timezone-and-ini"
    (expect
      (%php-run-capture
  ...
  (it-sequential
    ...
    (expect
      ...
      "Hi $n"))
  (it-sequential
    "php-e2e-list-destructuring"
    (expect (%php-run-capture "<?php list($a,$b)=[1,2]; echo $a+$b;") :to-equal "3")
    (expect
      (%php-run-capture "<?php list($a,$b,$c)=[10,20,30]; echo $a+$b+$c;")
      :to-equal
      "60")
    (expect
      (%php-run-capture
      ...
      "4-9")
    (expect
      (%php-run-capture "<?php list($n,$a)=['Bob',30]; echo $n.':'.$a;")
      :to-equal
      "Bob:30")
    (expect
      (%php-run-capture "<?php [$a,$b]=[1,2]; echo $a.','.$b;")
      :to-equal
      "1,2")
    (expect
      (%php-run-capture "<?php $a=1;$b=2; [$a,$b]=[$b,$a]; echo $a.','.$b;")
      :to-equal
      "2,1"))
  (it-sequential
    "php-e2e-list-destructuring-nested-keyed"
    (expect
      (%php-run-capture "<?php [[$a,$b],$c]=[[1,2],3]; echo $a+$b+$c;")
      :to-equal
      "6")
    (expect
      (%php-run-capture "<?php list(list($a,$b),$c)=[[1,2],3]; echo $a+$b+$c;")
      :to-equal
      "6")
    (expect
      (%php-run-capture "<?php ['x'=>$a,'y'=>$b]=['x'=>1,'y'=>2]; echo $a+$b;")
      :to-equal
      "3")
    (expect
      (%php-run-capture "<?php ['y'=>$a,'x'=>$b]=['x'=>1,'y'=>2]; echo $a-$b;")
      :to-equal
      "1")
    (expect
      (%php-run-capture "<?php [['k'=>$a],$b]=[['k'=>9],3]; echo $a.','.$b;")
      :to-equal
      "9,3"))
  (it-sequential
    "php-e2e-complex-string-interpolation"
    (expect
      (%php-run-capture "<?php $a=['k'=>5]; echo \"v={$a['k']}\";")
      :to-equal
      "v=5")
    (expect
      (%php-run-capture "<?php $a=['x'=>['y'=>9]]; echo \"n={$a['x']['y']}\";")
      :to-equal
      "n=9")
    (expect
      (%php-run-capture
    ...
    (expect
      ...
      "Hi Bob, age 30!")
    (expect
      (%php-run-capture "<?php $n='Bob'; echo \"Hi $n!\";")
      :to-equal
      "Hi Bob!")
    (expect
      (%php-run-capture "<?php $n='Bob'; echo \"Hi {$n}!\";")
      ...
      "Hi Bob!"))
  (it-sequential
    "php-e2e-equality-operators"
    (expect
      (%php-run-capture "<?php function f($x){ return $x == 4; } echo f(4);")
      :to-equal
      "1")
    (expect
      (%php-run-capture "<?php function f($x){ return $x == 4; } echo f(3);")
      :to-equal
      "")
    (expect
      (%php-run-capture "<?php function f($x){ return $x != 4; } echo f(3);")
      ...
      "1")
    (expect
      (%php-run-capture "<?php function f($x){ return $x % 2 == 0; } echo f(4);")
      :to-equal
      "1"))
  (it-sequential
    "php-e2e-equality-type-juggling"
    (expect
    ...
    (expect
      ...
      "strictfail"))
  (it-sequential
    "php-e2e-echo-boolean-php-semantics"
  ...
  (it-sequential
    "php-e2e-logical-operators"
    (expect (%php-run-capture "<?php echo (true && true) ? 'T':'F';") :to-equal "T")
    (expect
      (%php-run-capture "<?php echo (true && false) ? 'T':'F';")
      :to-equal
      "F")
    (expect
      (%php-run-capture "<?php echo (false || true) ? 'T':'F';")
      :to-equal
      "T")
    (expect
      (%php-run-capture "<?php echo (false || false) ? 'T':'F';")
      :to-equal
      "F")
  ...
  (it-sequential
    "php-e2e-logical-in-conditions"
    (expect
      (%php-run-capture "<?php $a=1; $b=2; if ($a && $b) { echo 'both'; }")
      :to-equal
      "both")
    (expect
      (%php-run-capture "<?php $x=5; if ($x > 0 && $x < 10) { echo 'mid'; }")
      :to-equal
      "mid")
    (expect
      (%php-run-capture "<?php $a=true; $b=false; if (!$b && $a) { echo 'ok'; }")
      :to-equal
      "ok"))
  (it-sequential
    ...
    (expect
      ...
      "F"))
  (it-sequential
    "php-e2e-default-arguments"
    (expect
      (%php-run-capture "<?php function g($x=7){ return $x; } echo g();")
      :to-equal
      "7")
    (expect
      (%php-run-capture "<?php function g($x=7){ return $x; } echo g(3);")
      :to-equal
      "3")
    (expect
      (%php-run-capture "<?php function g($s='hi'){ return $s; } echo g();")
      :to-equal
      "hi")
    (expect
      ...
      "15")
    (expect
      (%php-run-capture "<?php function f($a,$b=10){ return $a+$b; } echo f(5,2);")
      :to-equal
      "7")
    (expect
      (%php-run-capture
        "<?php function f($a=1,$b=2,$c=3){ return $a+$b+$c; } echo f();")
      :to-equal
      "6")
    (expect
      (%php-run-capture
        "<?php function f($a=1,$b=2,$c=3){ return $a+$b+$c; } echo f(10);")
      :to-equal
      "15"))
  (it-sequential
    "php-e2e-named-arguments"
    (expect
      (%php-run-capture
    ...
    (expect
      ...
      "ABC"))
  (it-sequential
    "php-e2e-default-arguments-all-callable-forms"
    (expect
      (%php-run-capture "<?php $f=function($x=9){ return $x; }; echo $f();")
      :to-equal
      "9")
    (expect
      (%php-run-capture "<?php $f=function($x=9){ return $x; }; echo $f(4);")
      :to-equal
      "4")
    ...
    (expect
      ...
      "20"))
  (it-sequential
    "php-e2e-variadic-parameters"
    (expect
      (%php-run-capture
        "<?php function s(...$n){ return array_sum($n); } echo s(1,2,3,4);")
      :to-equal
      "10")
    (expect
      (%php-run-capture
        "<?php function s(...$n){ return count($n); } echo s(1,2,3,4);")
      :to-equal
      "4")
    (expect
      (%php-run-capture "<?php function s(...$n){ return count($n); } echo s();")
      :to-equal
      "0")
    (expect
      (%php-run-capture
    ...
    (expect
      ...
      "abc"))
  (it-sequential
    "php-e2e-variadic-all-callable-forms"
    (expect
      (%php-run-capture
        "<?php $s=function(...$n){ return array_sum($n); }; echo $s(1,2,3);")
      :to-equal
      "6")
    (expect
      (%php-run-capture "<?php $s=fn(...$n)=>array_sum($n); echo $s(5,5,5);")
      :to-equal
      "15"))
  (it-sequential
    "php-e2e-spread-call"
  ...
  (it-sequential
    "php-e2e-named-args-after-dynamic-spread"
    (expect
      (%php-run-capture
    ...
    (expect
      ...
      "axc"))
  (it-sequential
    "php-e2e-spread-composes-with-variadic-and-closures"
    (expect
      (%php-run-capture
  ...
  (it-sequential
    ...
    (expect
      ...
      "6"))
  (it-sequential
    "php-e2e-for-loop-basics"
    (expect
      (%php-run-capture "<?php $s=0; for($i=1;$i<=4;$i++){ $s+=$i; } echo $s;")
      :to-equal
      "10")
    (expect
      (%php-run-capture
      ...
      "3")
    (expect
      (%php-run-capture "<?php $s=''; for($i=3;$i>0;$i--){ $s.=$i; } echo $s;")
      :to-equal
      "321"))
  (it-sequential
  ...
  (it-sequential
    "php-e2e-match-and-relational-booleans"
    (expect
      (%php-run-capture "<?php $x=2; echo match($x){1=>'a',2=>'b'};")
      :to-equal
      "b")
    (expect
      (%php-run-capture "<?php $x=2; echo match($x){1,2=>'low',3=>'hi'};")
      :to-equal
      "low")
    (expect
      (%php-run-capture "<?php $x=3; echo match($x){1,2=>'low',3=>'hi'};")
      :to-equal
      "hi")
    (expect
      (%php-run-capture "<?php $x=5; echo match(true){$x>3=>'big',default=>'small'};")
      :to-equal
      "big")
    (expect
      (%php-run-capture "<?php $x=1; echo match(true){$x>3=>'big',default=>'small'};")
      :to-equal
      "small")
    ...
    (expect
      (%php-run-capture "<?php echo 'apple'<'banana' ? 'y':'n';")
      :to-equal
      "y")
    (expect
      (%php-run-capture "<?php $s=0; for($i=0;$i<4;$i++){$s+=$i;} echo $s;")
      :to-equal
      "6")
    (expect (%php-run-capture "<?php echo 1<=>2;") :to-equal "-1"))
  (it-sequential
    "php-e2e-nullish-coalescing-assignment"
    (expect (%php-run-capture "<?php $a=null; $a ??= 'x'; echo $a;") :to-equal "x")
    (expect
      (%php-run-capture "<?php $a='keep'; $a ??= 'x'; echo $a;")
      :to-equal
      "keep")
    (expect
      (%php-run-capture "<?php $a=false; $a ??= 'x'; echo $a===false?'F':'o';")
      :to-equal
      "F")
    (expect (%php-run-capture "<?php $a=0; $a ??= 'x'; echo $a;") :to-equal "0")
    (expect
      (%php-run-capture
        "<?php class C{public $x;} $o=new C(); $o->x ??= 'y'; echo $o->x;")
      :to-equal
      "y")
    (expect
      (%php-run-capture
        "<?php class C{public $x=5;} $o=new C(); $o->x ??= 'y'; echo $o->x;")
      :to-equal
      "5")
    (expect
      (%php-run-capture "<?php $a=[]; $a['k'] ??= 'v'; echo $a['k'];")
      :to-equal
      "v")
    (expect
      (%php-run-capture "<?php $a=['k'=>3]; $a['k'] ??= 'v'; echo $a['k'];")
      :to-equal
      "3"))
  (it-sequential
    "php-e2e-compound-assign-undefined-var"
