...
(describe
  ...
  (it-sequential
    ...
    (expect
      ...
      "3,2,1"))
  (it-sequential
    "php-e2e-array-column-basic-and-index"
    (expect
      (%php-run-capture
  ...
  (it-sequential
    ...
    (expect
      ...
      "Ada,Bob"))
  (it-sequential
    "php-e2e-array-keys-filters-values"
    (expect
      (%php-run-capture
    ...
    (expect
      ...
      "x"))
  (it-sequential
    "php-e2e-array-search-strict-and-miss"
    (expect
      (%php-run-capture
  ...
  (it-sequential
    ...
    (expect
      ...
      "3"))
  (it-sequential
    "php-e2e-array-filter-mode-flags"
    (expect
      (%php-run-capture
  ...
  (it-sequential
    ...
    (expect
      ...
      "x:1|0:2|"))
  (it-sequential
    "php-e2e-array-append"
    (expect
      (%php-run-capture "<?php $a=[1,2]; $a[]=3; echo count($a);")
      :to-equal
      "3")
    (expect (%php-run-capture "<?php $a=[1,2]; $a[]=3; echo $a[2];") :to-equal "3")
    (expect
      (%php-run-capture "<?php $a=[]; $a[]='x'; $a[]='y'; echo $a[0].$a[1];")
      :to-equal
      "xy")
    (expect
      (%php-run-capture
      ...
      "01020")
    (expect
      (%php-run-capture "<?php $a=['k'=>1]; $a[]=2; echo $a['k'].$a[0];")
      :to-equal
      "12"))
  (it-sequential
    "php-e2e-array-subscript-set-still-works"
    (expect (%php-run-capture "<?php $a=[1]; $a[0]=9; echo $a[0];") :to-equal "9")
    (expect
      (%php-run-capture "<?php $a=[]; $a['x']=5; echo $a['x'];")
      :to-equal
      "5"))
  (it-sequential
  ...
  (it-sequential
    "php-e2e-array-spread"
    (expect
      (%php-run-capture "<?php $a=[1,2]; $b=[...$a,3]; echo array_sum($b);")
      :to-equal
      "6")
    (expect
      (%php-run-capture "<?php $a=[2,3]; $b=[1,...$a,4]; echo implode(',',$b);")
      :to-equal
      "1,2,3,4")
    (expect
      (%php-run-capture "<?php $a=[1]; $b=[2]; $c=[...$a,...$b]; echo count($c);")
      :to-equal
      "2")
    (expect
      (%php-run-capture "<?php $a=[5,6]; $b=[...$a]; echo array_sum($b);")
      :to-equal
      "11")
    (expect
      (%php-run-capture "<?php $a=['x'=>1]; $b=[...$a,'y'=>2]; echo $b['x'].$b['y'];")
      :to-equal
      "12")
    (expect
      (%php-run-capture "<?php $a=[5=>1, 9=>2]; $b=[...$a]; echo $b[0].$b[1];")
      :to-equal
      "12")
  ...
  (it-sequential
    "php-e2e-array-union"
    (expect
      (%php-run-capture "<?php $a=[1,2]+[3,4,5]; echo count($a);")
      :to-equal
      "3")
    (expect
      (%php-run-capture "<?php $a=[1,2]+[3,4,5]; echo implode(',',$a);")
      :to-equal
      "1,2,5")
    (expect
      (%php-run-capture "<?php $a=['x'=>1]+['x'=>9,'y'=>2]; echo $a['x'].$a['y'];")
      :to-equal
      "12")
    (expect
      (%php-run-capture "<?php $a=[1=>'a']+[1=>'b',2=>'c']; echo $a[1].$a[2];")
      :to-equal
      "ac")
    ...
    (expect
      (%php-run-capture "<?php echo count(array_merge([1,2],[3,4]));")
      :to-equal
      "4"))
  (it-sequential
    "php-e2e-intdiv-fdiv-array-is-list"
    ...
    (expect
      (%php-run-capture "<?php echo array_is_list([1,2,3])?'y':'n';")
      :to-equal
      "y")
    (expect
      (%php-run-capture "<?php echo array_is_list([1=>'a',0=>'b'])?'y':'n';")
      :to-equal
      "n")
    (expect
      (%php-run-capture "<?php echo array_is_list(['x'=>1])?'y':'n';")
      :to-equal
      "n")
    (expect
      (%php-run-capture "<?php echo array_is_list([])?'y':'n';")
      :to-equal
      "y"))
  (it-sequential
    "php-e2e-usort-in-place"
    (expect
      (%php-run-capture
    ...
    (expect
      ...
      "cab"))
  (it-sequential
    "php-e2e-array-multisort-syncs-arrays-and-flags"
    (expect
      (%php-run-capture
    ...
    (expect
      ...
      "third,first|0,1"))
  (it-sequential
    "php-e2e-sort-builtins-honor-sort-flags"
    (expect
      (%php-run-capture
  ...
  (it-sequential
    ...
    (expect
      ...
      "Img1,img2,IMG10")
    (expect
      (%php-run-capture "<?php echo strnatcmp('img2','img10');")
      :to-equal
      "-1")
    (expect
      (%php-run-capture "<?php echo strnatcmp('img10','img2');")
      :to-equal
      "1")
    (expect (%php-run-capture "<?php echo strnatcmp('abc','abc');") :to-equal "0")
    (expect
      (%php-run-capture "<?php echo strnatcasecmp('IMG2','img10');")
      :to-equal
      "-1"))
  (it-sequential
    "php-e2e-array-merge-recursive"
    (expect
      (%php-run-capture
